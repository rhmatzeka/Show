...
export default function HomePage() {
    ...
    if (projects.length > 0 && scrollContainerRef.current) {
      ...
      const adjustScroll = () => {
        ...
        if (setElements.length >= 3) {
          ...
          // Initialize scroll values for inertia scroll syncing
  ...
  const renderProjectCard = (project: ProjectData, index: number, setId: number) => {
    const itemId = `item-${project.slug}-${setId}`;
    const isHidden = hiddenCardId === itemId;
    const coverImg = project.images.find(img => img.isCover) || project.images[0];

    // Keep the custom interactive SVG logo for symbol-card
    if (project.slug === "symbol-card") {
      return (
        <div
          key={itemId}
    ...
    // Keep the custom layout with side panel for Einstoffen
    if (project.slug === "einstoffen") {
      return (
        <div
          key={itemId}
          ref={(el) => { projectRefs.current[itemId] = el; }}
    ...
    // Keep the red color-blend overlay for Rampant Studio
    if (project.slug === "rampant-studio") {
      return (
        <div
          key={itemId}
          ref={(el) => { projectRefs.current[itemId] = el; }}
    ...
    // Keep the custom offset headings for Coleção
    if (project.slug === "colecao") {
      return (
        <div
          key={itemId}
          ref={(el) => { projectRefs.current[itemId] = el; }}
    ...
    // Keep the custom alphabet card if slug matches alphabet-card
    if (project.slug === "alphabet-card") {
      return (
        <div
          key={itemId}
          ref={(el) => { projectRefs.current[itemId] = el; }}
    ...
    // Keep AB Arca custom SVG
    if (project.slug === "ab-arca") {
      return (
        <div
          key={itemId}
          ref={(el) => { projectRefs.current[itemId] = el; }}
    ...
    // Keep Brand Identity MM26 bars
    if (project.slug === "brand-identity-mm26") {
      return (
        <div
          key={itemId}
          ref={(el) => { projectRefs.current[itemId] = el; }}
    ...
    // Keep Macbeth theater text
    if (project.slug === "macbeth") {
      return (
        <div
          key={itemId}
          ref={(el) => { projectRefs.current[itemId] = el; }}
    ...
    // Default template rendering for any new/custom dynamic project uploaded in Admin CMS
    ...
    return (
      <div
        ...
        ref={(el) => { projectRefs.current[itemId] = el; }}
        onClick={() => openProject(project, itemId)}
        className={`bg-white group cursor-pointer overflow-hidden project-card-interactive shadow-sm transition-colors w-full ${aspectClass} relative`}
      >
        <div className={`w-full h-full relative flex flex-col justify-between transition-opacity duration-200 project-card-interactive-content ${isHidden ? "opacity-0" : "opacity-100"}`}>
          {coverImg ? (
            <div className="w-full h-full relative overflow-hidden bg-black/5 flip-source-el">
              <Image
                ...
                className="object-cover grayscale contrast-115 group-hover:scale-[1.03] transition-transform duration-700 ease-out"
              />
              <div className="absolute bottom-4 left-4 z-10 bg-black/85 px-3 py-1.5 border border-white/20">
                <h3 className="font-sans font-black text-xs md:text-sm text-white uppercase tracking-wider">
                  {project.title}
                </h3>
                <p className="text-[8px] md:text-[10px] font-mono text-white/60 uppercase tracking-widest mt-0.5">
            ...
            <div className="w-full h-full bg-[#0B0B0B] flex flex-col justify-between p-4 flip-source-el">
              <div className="flex justify-between items-start">
                <span className="text-[8px] md:text-[10px] font-bold text-white/50 uppercase">{project.category}</span>
                <span className="text-[8px] md:text-[10px] font-bold text-white/50">{project.year}</span>
              </div>
              <div className="my-auto flex justify-center items-center text-center px-4">
                <h3 className="font-display font-black text-lg md:text-2xl text-white uppercase tracking-wider leading-none">
                  {project.title}
                </h3>
              </div>
              <div className="flex justify-between items-end">
                <span className="text-[8px] md:text-[9px] font-mono text-white/40">PROJECT</span>
                <span className="text-[8px] md:text-[9px] font-mono text-white/40">© RAHMAT</span>
              </div>
            </div>
          )}
        </div>
  ...
  const renderCardsSet = (setId: number) => {
    // Filter out symbol-card from homepage main listing
    const filteredProjects = projects.filter(p => p.slug !== "symbol-card");
    
    // Distribute fetched database projects dynamically across 4 columns
    const col1 = filteredProjects.filter((_, idx) => idx % 4 === 0);
    const col2 = filteredProjects.filter((_, idx) => idx % 4 === 1);
    const col3 = filteredProjects.filter((_, idx) => idx % 4 === 2);
    const col4 = filteredProjects.filter((_, idx) => idx % 4 === 3);

    return (
      <div className="grid grid-cols-2 md:grid-cols-4 gap-8 md:gap-16 w-full px-6 md:px-12 md:pl-24">
        {/* Column 1 */}
        <div className="flex flex-col gap-12 md:gap-24">
          {col1.map((project, idx) => renderProjectCard(project, idx * 4, setId))}
        </div>

        {/* Column 2 */}
        <div className="flex flex-col gap-12 md:gap-24 pt-16 md:pt-32">
          {col2.map((project, idx) => renderProjectCard(project, idx * 4 + 1, setId))}
        </div>

        {/* Column 3 */}
        <div className="flex flex-col gap-12 md:gap-24 pt-8 md:pt-16">
          {col3.map((project, idx) => renderProjectCard(project, idx * 4 + 2, setId))}
        </div>

        {/* Column 4 */}
        <div className="flex flex-col gap-12 md:gap-24 pt-24 md:pt-48">
          {col4.map((project, idx) => renderProjectCard(project, idx * 4 + 3, setId))}
        </div>
      </div>
    );
  };
